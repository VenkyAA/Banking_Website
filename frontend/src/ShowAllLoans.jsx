...
const ShowAllLoans = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (userDetails) {
        const token = userDetails.token;
        await axios.delete(`http://localhost:8765/loans/${loanToDelete.loanId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        fetchLoans();
        setShowModal(false);
      ...
      setError('There was an error deleting the loan!');
    }
  };
                ...
                {data.map((loan, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      ...
                      <button className="btn btn-danger" onClick={() => handleDelete(loan)}>
                        DELETE
                      </button>
                    </td>
                  </tr>
