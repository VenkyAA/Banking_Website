...
const ApplyForLoan = () => {
  ...
  useEffect(() => {
    if (cibilScore && principalAmount && numberOfYears) {
      // Calculate interest rate and amount to be repaid here if necessary
    }
  ...
  const handleLoan = async () => {
    if (!loanType || !principalAmount || !numberOfYears || !cibilScore || !amountPaid) {
    ...
    try {
      ...
      if (userDetails) {
        ...
        const response = await axios.post('http://localhost:8765/loans', loanData, {
      ...
      <div className="card mt-5">
        <div className="card-body">
          ...
          <form>
            ...
            <button type="button" className="btn btn-info" onClick={handleLoan} disabled={isLoading}>
              {isLoading ? 'Applying...' : 'Apply'}
            </button>
            <button type="reset" className="btn btn-secondary" onClick={resetForm} disabled={isLoading}>
              Cancel
            </button>
          </form>
        </div>
